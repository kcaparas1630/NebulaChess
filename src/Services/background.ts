...
chrome.runtime.onMessage.addListener(
  (request, _sender, sendResponse: (response: MessageResponse) => void) => {
    ...
    if (request.type === "REQUEST_ANALYSIS") {
      ...
      analyzePosition(request.fen, request.playerToMove, request.moveHistory)
  ...
  try {
    ...
    if (moveNotation.startsWith('B') && !moveNotation.includes('x')) {
      ...
      for (const [bishopRank, bishopFile] of bishops) {
        ...
        if (Math.abs(bishopRank - destRank) === Math.abs(bishopFile - destFile)) {
...
const validateFen = (fen: string) => {
  ...
  if (whiteKingCount !== 1) {
    return { valid: false, error: `There must be exactly one white king, found ${whiteKingCount}` };
  }
  
  if (blackKingCount !== 1) {
    return { valid: false, error: `There must be exactly one black king, found ${blackKingCount}` };
  }
...
const analyzePawnStructure = (ranks: string[]): string => {
  ...
  try {
    ...
    if (!hasFeatures) {
      ...
      analysis += `\n- White pawns on files: ${[...new Set(whitePawnFiles)].sort().join(', ') || 'none'}`;
      analysis += `\n- Black pawns on files: ${[...new Set(blackPawnFiles)].sort().join(', ') || 'none'}`;
    }
...
const createEnhancedContext = (fen: string, moveHistory: (string | ChessMove)[]) => {
...
const analyzePosition = async (
  ...
  try {
    ...
    try {
      ...
      if (!isValidMove) {
        console.warn(`AI suggested invalid move: ${responseData.bestMove} - replacing with fallback move`);
        responseData.bestMove = "Nc3";
        responseData.moveReasoning = "Developing a knight to a good central square, controlling important central squares and preparing for further development.";
      }
      ...
      return {
        ...
        moveReasoning: "The analysis engine returned an invalid response. Please try again.",
        depth: 0,
      };
    }
  } catch (error: unknown) {
